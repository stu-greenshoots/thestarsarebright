...
class Hustle_Module_Model extends Hustle_Model {
	...
	public function sanitize_form_elements( $form_elements ) {
		...
		foreach ( $form_elements as $field_data ) {
			$name = self::sanitize_form_field_name( $field_data['name'] );

			// After sanitize if name become empty then create array key from label.
			if ( ! $name ) {
				$name = self::sanitize_form_field_name( $field_data['label'] );
			...
			$name = self::get_unique_field_name( $sanitized_fields, $name );
	 ...
	 * @return array
	 */
	public static function get_unique_field_name( $sanitized_fields, $field_name ) {
		$new_name = $field_name;
		$i        = 0;
