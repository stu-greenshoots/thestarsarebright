		...
		public function add_plugin_action_links( $actions, $plugin_file ) {
			...
			if ( $plugin === $plugin_file ) {
				...
				if ( Opt_In_Utils::_is_free() ) {
					if ( 'full' !== Opt_In_Utils::get_membership_status() ) {
				...
				if ( current_user_can( 'activate_plugins' ) && ( ! is_network_admin() || ! is_multisite() ) ) {
					...
					if ( $has_404_backup ) {
						...
						/* translators: Hustle version to rollback to */
						$links['rollback_404'] = '<a href="' . esc_url_raw( $url ) . '">' . sprintf( esc_html__( 'Rollback to %s', 'hustle' ), $version ) . '</a>';
					}
